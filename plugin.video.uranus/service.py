...
addon_path = xbmc.translatePath(('special://home/addons/plugin.video.uranus')).decode('utf-8')
...
def main():
    ...
    updated = xbmcaddon.Addon(id='plugin.video.uranus').getSetting('module_base')
    ...
    log('uranus Scraper Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
        log('uranus Service - Exception: \n %s' % (str(failure)), LOGINFO)
        return
    ...
    xbmcaddon.Addon(id='plugin.video.uranus').setSetting('module_base', fum_ver)
    ...
    log('uranus Path: %s' % (str(scraper_path)), LOGINFO)
    try:
        xml = openfile(settings_xml_path)
    except Exception:
        failure = traceback.format_exc()
...
DEBUGPREFIX = '[COLOR red][ uranus DEBUG ][/COLOR]'

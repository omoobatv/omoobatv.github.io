...
def rules():
    regexes = plugin.get_storage('regexes')
    items = []
    for (regex,path),label in regexes.items():
        #log((regex,path))
        context_items = []
...
def favourite_folders():
    ...
    items = []
    for path,label in favourites.items():
        #log((regex,path))
        context_items = []
...
def service_folder(regex,path,label,depth=1):
    ...
    for f in files:
        ...
        if f['filetype'] == 'directory':
            ...
            if record_label not in list(recordings.values()):
...
def do_find_folder(regex,path,label,depth=1,mark=False):
    ...
    for f in files:
        ...
        if f['filetype'] == 'directory':
            ...
            if record_label in list(recordings.values()):
                recorded = True
            else:
                recorded = False
...
def folder(path,label):
    ...
    for f in files:
        ...
        if f['filetype'] == 'directory':
            ...
            dir_items.append({
                'label': dir_label,
                'path': plugin.url_for('folder', path=url, label=file_label),
                'thumbnail': f['thumbnail'],
                'context_menu': context_items,
            })
        else:
...
def cancel_recordings():
    ...
    recordings_labels = [(k,v) for k,v in sorted(iter(recordings.items()),key=lambda x: x[1])]
...
def clear_folders():
    ...
    folders_labels = [(k,v) for k,v in sorted(iter(folders.items()),key=lambda x: x[1])]
...
def clear_trakt_shows():
    ...
    trakt_shows_labels = [(k,v) for k,v in sorted(iter(trakt_shows.items()),key=lambda x: x[1])]
...
def clear_trakt_movies():
    ...
    trakt_movies_labels = [(k,v) for k,v in sorted(iter(trakt_movies.items()),key=lambda x: x[1])]
...
def browse(table):
    conn = sqlite3.connect(xbmcvfs.translatePath('special://profile/addon_data/%s/replay.db' % addon_id()), detect_types=sqlite3.PARSE_DECLTYPES)
    c = conn.cursor()

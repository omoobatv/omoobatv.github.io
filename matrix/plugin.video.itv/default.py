...
def SHOWS(url):
    ...
    #print "BUF %s" % buf
    f.close()
    buf = buf.split('cp_grid__item cp_tile-grid__item')
    buf.pop(0)
    image='https://hubimages.itv.com/episode/10_0675_0002?w=466&q=80&blur=0&bg=false'
    for p in buf:
        try:
            ...
            #print image
            name= re.compile('data-testid=\"basic-tile-title\">(.+?)</h3',re.DOTALL).findall (p)[0].strip()
            print (name)
            episodes = re.compile('<span class=\"cp_basic-tile__episode-count\"><span>(.+?)<!--',re.DOTALL).findall (p)[0].strip()
            if 'mins' in episodes:
            ...
            if 'day left' in episodes or 'days left' in episodes or episodes=='1 episode' or 'mins' in episodes:
                if not 'mins' in episodes:
                    linkurl = linkurl+'##'
                addDir2(name+' - [COLOR orange]%s[/COLOR]'%episodes,'https:'+linkurl,3,'', '',image,'',isFolder=False)
            else:
                if not 'no episodes' in episodes.lower():
                    addDir(name+' - [COLOR orange]%s[/COLOR]'%episodes,'https:'+linkurl,2,image)
        except:pass
